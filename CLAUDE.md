...
`lambda/quality.py` — pure check functions (no AWS deps, no Polars), imported by `lambda_iceberg_writer.py` as `quality` module. Operates on `list[dict[str, Any]]`.
